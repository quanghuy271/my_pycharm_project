...
# ===============================
# Đọc file CSV bão và lấy list timestamp
# ==========================================================
def load_time_from_csv(csv_file):
    df = pd.read_csv(csv_file)

    if "thoi_gian" not in df.columns:
        raise KeyError("File CSV không có cột 'thoi_gian'")

    df["thoi_gian"] = pd.to_datetime(df["thoi_gian"], format="%d/%m/%Y %H:%M")

    time_set = set(df["thoi_gian"])

    print("Tổng số dòng trong CSV:", len(df))
    print("Số mốc thời gian duy nhất:", len(time_set))

    return sorted(list(time_set))
# ==========================================================
# Pipeline tải ERA5 theo đúng timestamp trong file CSV
# ==========================================================
def era5_pipeline_from_csv(
        csv_file,
        base_dir="ERA5",
        variables=["t"],
        pressure_level=None
):
    timestamps = load_time_from_csv(csv_file)
...
if __name__ == "__main__":
    era5_pipeline_from_csv(
        csv_file="tat_ca_con_bao_1940.csv",  # đổi thành tên file của bạn
        base_dir="ERA5",
        variables=[
            "t", "z",
            "q", "r", "vo",
